...
class DiffUtilSliverList<T> extends StatefulWidget {
  ...
  @override
  DiffUtilSliverListState<T> createState() => DiffUtilSliverListState<T>();
}

class DiffUtilSliverListState<T> extends State<DiffUtilSliverList<T>> {
  ...
  void _onDiffUpdate(diffutil.DiffUpdate update, List<T?> tmpList) {
    update.when<void>(
      move: (_, __) => throw UnimplementedError('moves are currently not supported'),
      insert: (pos, count) => _onInserted(pos, count, tmpList),
      change: (pos, payload) => throw UnimplementedError('changes are currently not supported'),
